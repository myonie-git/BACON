...
int main(int argc, char *argv[]) {
    ...
    int n = model.collisionGeometries.size(); // 元素数量
    ...
    for (int element_index = 0; element_index < n; ++element_index) {
        ...
        // 设定蒙特卡洛树搜索的参数
        // int max_iterations = 10000;    // 每个元素的最大迭代次数
        // int time_limit_ms = 50000;      // 每个元素的时间限制（毫秒）
                            ...
                            for (int i4 = 0; i4 < steps; i4++) {
                                for (int i5 = 0; i5 < steps; i5++) {
                                    for (int i6 = 0; i6 < steps; i6++) {
                                        ...
                                        // 采样 6 个关节（与 openmp_sim_cpu_go1.cpp 的映射保持一致）
                                        local_jointAngles["FL_calf_joint"] = joint1;
                                        local_jointAngles["FL_hip_joint"] = joint2;
                                        local_jointAngles["FL_thigh_joint"] = joint3;
                                        local_jointAngles["FR_calf_joint"] = joint4;
                                        local_jointAngles["FR_hip_joint"] = joint5;
                                        local_jointAngles["FR_thigh_joint"] = joint6;
