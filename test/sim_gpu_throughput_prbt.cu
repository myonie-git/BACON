...
// // Function to check collision on GPU with added timing
// void checkcollisionOnGPU(const OBB_GPU* h_collision1_obb, size_t num_i, const OBB_GPU* d_collision2_obb, size_t num_j, int* h_results, int* d_results, size_t batch_size,
//                          double& gpu_data_transfer_time, double& gpu_kernel_time, double& gpu_copy_back_time) {
//     size_t total = num_i * num_j * batch_size;

//     // Allocate device memory for collision geometry
//     OBB_GPU* d_collision1_obb;
//     cudaEvent_t start_transfer_h2d, end_transfer_h2d;
//     cudaEventCreate(&start_transfer_h2d);
//     cudaEventCreate(&end_transfer_h2d);

//     cudaEventRecord(start_transfer_h2d);
//     cudaMalloc((void**)&d_collision1_obb, num_i * batch_size * sizeof(OBB_GPU));

//     // Copy h_collision1_obb to device
//     cudaMemcpy(d_collision1_obb, h_collision1_obb, num_i * batch_size * sizeof(OBB_GPU), cudaMemcpyHostToDevice);
//     cudaEventRecord(end_transfer_h2d);
//     cudaEventSynchronize(end_transfer_h2d);

//     float milliseconds_h2d = 0;
//     cudaEventElapsedTime(&milliseconds_h2d, start_transfer_h2d, end_transfer_h2d);
//     gpu_data_transfer_time += milliseconds_h2d / 1000.0; // Convert to seconds

//     cudaEventDestroy(start_transfer_h2d);
//     cudaEventDestroy(end_transfer_h2d);
...
//     cudaEventElapsedTime(&milliseconds_kernel, start_kernel, end_kernel);
//     gpu_kernel_time += milliseconds_kernel / 1000.0; // Convert to seconds

//     cudaEventDestroy(start_kernel);
//     cudaEventDestroy(end_kernel);

//     // Copy results back with timing
//     cudaEvent_t start_copy_back, end_copy_back;
//     cudaEventCreate(&start_copy_back);
//     cudaEventCreate(&end_copy_back);

//     cudaEventRecord(start_copy_back);
//     cudaMemcpy(h_results, d_results, batch_size * sizeof(int), cudaMemcpyDeviceToHost);
//     cudaEventRecord(end_copy_back);
//     cudaEventSynchronize(end_copy_back);

//     float milliseconds_copy_back = 0;
//     cudaEventElapsedTime(&milliseconds_copy_back, start_copy_back, end_copy_back);
//     gpu_copy_back_time += milliseconds_copy_back / 1000.0; // Convert to seconds

//     cudaEventDestroy(start_copy_back);
//     cudaEventDestroy(end_copy_back);
...
// // Main function with added timing instrumentation
// int main(int argc, char *argv[]) {
...
//     // Initialize total timing variables
//     double total_file_read_time = 0.0;
//     double total_file_duration = 0.0;
//     double total_type_conversion_time = 0.0;
//     double total_prepare_joint_configs_time = 0.0;
//     double total_prepare_host_obb_time = 0.0;
//     double total_gpu_data_transfer_time = 0.0;
//     double total_gpu_kernel_time = 0.0;
//     double total_gpu_copy_back_time = 0.0;
//     double total_cuda_malloc_time = 0.0;
...
//             // Start timing file read and type conversion
//             std::vector<CollisionObject<S>*> collision_objects = readBoxesFromFile<S>(filename);
...
//             // Start timing type conversion to h_collision2_obb
//             auto type_conversion_start = std::chrono::high_resolution_clock::now();
//             std::vector<OBB_GPU> h_collision2_obb(num_j);
...
//                 }
//             }
//             std::chrono::duration<double> type_conversion_duration = type_conversion_end - type_conversion_start;
//             total_type_conversion_time += type_conversion_duration.count();

//             // Allocate device memory for collision_objects and copy 
            
//             auto cuda_malloc_start = std::chrono::high_resolution_clock::now();
//             OBB_GPU* d_collision2_obb;
//             cudaMalloc((void**)&d_collision2_obb, num_j * sizeof(OBB_GPU));
//             cudaMemcpy(d_collision2_obb, h_collision2_obb.data(), num_j * sizeof(OBB_GPU), cudaMemcpyHostToDevice);
//             auto cuda_malloc_end = std::chrono::high_resolution_clock::now();
//             std::chrono::duration<double> cuda_malloc_duration = cuda_malloc_end - cuda_malloc_start;
//             total_cuda_malloc_time += cuda_malloc_duration.count();

//             // Assume collision_geometry_ size is constant
//             size_t num_i = model.collisionGeometries.size();

//             // Start timing joint angle configuration preparation
//             auto joint_config_start = std::chrono::high_resolution_clock::now();
...
//             auto joint_config_end = std::chrono::high_resolution_clock::now();
//             std::chrono::duration<double> joint_config_duration = joint_config_end - joint_config_start;
//             total_prepare_joint_configs_time += joint_config_duration.count();
...
//                 // Start timing host OBB preparation
//                 auto host_obb_start = std::chrono::high_resolution_clock::now();
...
//                 auto host_obb_end = std::chrono::high_resolution_clock::now();
//                 std::chrono::duration<double> host_obb_duration = host_obb_end - host_obb_start;
//                 total_prepare_host_obb_time += host_obb_duration.count();

//                 // Perform collision detection for the batch with timing
//                 double gpu_data_transfer_time = 0.0;
//                 double gpu_kernel_time = 0.0;
//                 double gpu_copy_back_time = 0.0;

//                 checkcollisionOnGPU(h_collision1_obb.data(), num_i, d_collision2_obb, num_j, h_results.data(), d_results, current_batch_size,
//                                    gpu_data_transfer_time, gpu_kernel_time, gpu_copy_back_time);

//                 // Accumulate GPU timings
//                 total_gpu_data_transfer_time += gpu_data_transfer_time;
//                 total_gpu_kernel_time += gpu_kernel_time;
//                 total_gpu_copy_back_time += gpu_copy_back_time;

//                 // Process results (optional timing can be added here if needed)
//                 // for (size_t config_idx = 0; config_idx < current_batch_size; ++config_idx) {
//                 //     if (h_results[config_idx]) {
//                 //         // Handle collision
//                 //     }
...
//             std::chrono::duration<double> fire_duration = file_end_time - file_start_time;
//             total_file_duration += fire_duration.count();
//             // Optionally, accumulate per-file durations if needed
//         }
//     }

//     auto overall_end_time = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> overall_duration = overall_end_time - overall_start_time;

//     // Output the timing statistics
//     std::cout << "Timing Statistics:" << std::endl;
//     std::cout << "-------------------" << std::endl;
//     std::cout << "File Duration Time " << total_file_duration << std::endl;
//     std::cout << "Cuda Malloc Time " << total_cuda_malloc_time << std::endl;
//     std::cout << "Total File Read Time: " << total_file_read_time << " seconds" << std::endl;
//     std::cout << "Total Type Conversion Time: " << total_type_conversion_time << " seconds" << std::endl;
//     std::cout << "Total Joint Configuration Preparation Time: " << total_prepare_joint_configs_time << " seconds" << std::endl;
//     std::cout << "Total Host OBB Preparation Time: " << total_prepare_host_obb_time << " seconds" << std::endl;
//     std::cout << "Total GPU Data Transfer Time (H2D + D2H): " << total_gpu_data_transfer_time + total_gpu_copy_back_time << " seconds" << std::endl;
//     std::cout << "   - H2D Transfer Time: " << total_gpu_data_transfer_time << " seconds" << std::endl;
//     std::cout << "   - D2H Copy Back Time: " << total_gpu_copy_back_time << " seconds" << std::endl;
//     std::cout << "Total GPU Kernel Execution Time: " << total_gpu_kernel_time << " seconds" << std::endl;
//     std::cout << "Overall Execution Time: " << overall_duration.count() << " seconds" << std::endl;
