...
int main(){
    ...
    std::ofstream outfile(BACON_SOURCE_DIR "/data/compute_top_tf_ram_results.txt");
    if (!outfile.is_open()) {
    ...
    for(int i = 0; i < 1; i++){
        ...
        model.calculateWorldCoordinates_toram(outfile, rootLink);
    }

    jointfile.close();
    outfile.close();
    std::cout << "Random tree traversal test data and results have been written to compute_top_tf_ram_results.txt" << std::endl;
