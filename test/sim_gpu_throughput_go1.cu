        ...
        if (entry.is_regular_file()) {
            ...
            for (size_t batch_start = 0; batch_start < total_configs; batch_start += batch_size) {
                ...
                for (size_t config_idx = 0; config_idx < current_batch_size; ++config_idx) {
                    ...
                    jointAngles["FL_calf_joint"] = joint_angles[0];
                    jointAngles["FR_hip_joint"] = joint_angles[1];
                    jointAngles["RL_calf_joint"] = joint_angles[2];
                    jointAngles["RR_calf_joint"] = joint_angles[3];
                    jointAngles["FL_thigh_joint"] = joint_angles[4];
                    jointAngles["FR_hip_joint"] = joint_angles[5];
