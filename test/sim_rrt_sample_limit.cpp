...
{
    out << "用法: " << (argv0 ? argv0 : "sim_rrt_sample_limit") << " [options]\n\n"
        << "常用参数:\n"
        ...
        << "\n规划参数:\n"
        << "  --threads <n>           兼容参数；RRT 为单线程，该参数会被忽略(默认: min(4, CPU核数))\n"
        << "  --range <r>             扩展步长(0=>自动, 默认: 0)\n"
        << "  --goal-bias <p>         采样到 goal 的概率(默认: 0.05)\n"
        << "  --samples <n>           固定采样次数N，达到后立即终止(默认: 50000)\n"
        << "  --time <sec>            固定运行时间(秒)，到点后立即终止（与 --samples 互斥）\n"
        << "  --times <sec>           同 --time（兼容写法，与 --samples 互斥）\n"
...
// 与本仓库的 pRRT 修改一致：即使找到解，也继续扩展直到 ptc（这里由 sample limit 驱动）触发。
// 这样才能保证“达到期望采样次数之后才返回”，用于固定采样次数的性能对比实验。
class FixedSampleRRT : public og::RRT
{
public:
    explicit FixedSampleRRT(const ob::SpaceInformationPtr &si, bool addIntermediateStates = false)
    ...
    ob::PlannerStatus solve(const ob::PlannerTerminationCondition &ptc) override
    {
        ...
        // 关键差异：不在首次满足 goal 时 break，而是持续运行到 ptc 触发。
        while (!ptc)
        {
            ...
            if (si_->checkMotion(nmotion->state, dstate))
            {
                ...
                double dist = 0.0;
                const bool sat = goal->isSatisfied(nmotion->state, &dist);
                if (sat)
                {
                    // 保留第一次或更优的解，但不提前退出。
...
int main(int argc, char **argv)
{
    ...
    static const option longOpts[] = {
        ...
        {"times", required_argument, nullptr, OPT_TIME},
    ...
    if (timeProvided && samplesProvided)
    {
        std::cerr << "ERROR: --samples 与 --time/--times 互斥，请只设置其中一个。\n";
        printUsage(std::cerr, argv[0]);
        return 2;
    }
    ...
    if (timeProvided)
    {
        if (!(timeLimitSec > 0.0))
        {
            std::cerr << "ERROR: --time/--times 必须大于 0 (单位: 秒)\n";
            return 2;
        }
    }
    else
    {
        // 默认使用 samples 模式；未显式提供时采用默认 sampleLimit=50000
        if (sampleLimit == 0)
        {
            std::cerr << "采样次数必须大于 0\n";
            return 2;
    ...
    if (timeProvided && sampleLimit == 0)
    {
        // 防御：time 模式下 sampleLimit 不参与终止条件，这里避免后续打印/除法边界问题
        sampleLimit = 1;
    }
    ...
    std::cout << "Planner: RRT (threads=" << threads << " (ignored), range=" << range << ", goal_bias=" << goalBias << ")\n";
    std::cout << "Robot : " << spec.id << "\nURDF  : " << spec.urdf << "\nRoot  : " << spec.rootLink
              << "\nEnv   : " << envFile << " (boxes=" << envPtrs.size() << ")\n";
    std::cout << "DoF   : " << dof << "\nBounds: [" << jointLow << ", " << jointHigh << "]\n";
    if (timeProvided)
        std::cout << "Time limit T : " << timeLimitSec << " s\n";
    else
    ...
    auto planner = std::make_shared<FixedSampleRRT>(si);
    ...
    // 重置统计，确保仅统计 solve 阶段
    // 注意：OMPL 在 range=0(自动) 时，会在 setup() 阶段做若干次随机采样来估计合适的 range。
    // 这些采样会经过我们的 CountingSampler，导致 stats->samples 在 solve() 前就被累加（典型为 100 次），
    // 从而在 samples 很小（例如 20/50/100）时，终止条件一开始就为真，solve 几乎不运行，valid_calls 变成 1。
    // 为了实现“达到期望采样次数之后才返回”，这里将采样统计在 setup() 后清零，只统计 solve 阶段的采样。
    stats->samples.store(0, std::memory_order_relaxed);
    stats->sampleNs.store(0, std::memory_order_relaxed);
    ThreadSafeEnvCollisionChecker::resetStats();

    // 停止条件（二选一）：
    // - samples 模式：采样次数达到 N 即终止（可能略有超出）。
    // - time 模式：墙钟时间达到 T 秒即终止。
    const ob::PlannerTerminationCondition ptc =
        timeProvided ? ob::timedPlannerTerminationCondition(timeLimitSec)
                     : ob::PlannerTerminationCondition([&stats, sampleLimit]() {
    ...
    if (hasPath)
    {
        if (timeProvided)
        {
            std::cout << "Terminated after reaching time limit (T=" << timeLimitSec << "s)，已找到路径"
                      << (solved.asString() == std::string("Exact solution") ? "。\n" : "（近似解）。\n");
        }
        else
        {
            std::cout << "Terminated after reaching sample limit (N=" << sampleLimit << ")，已找到路径"
                      << (solved.asString() == std::string("Exact solution") ? "。\n" : "（近似解）。\n");
        }
    }
    else
    {
        if (timeProvided)
            std::cout << "Terminated after reaching time limit (T=" << timeLimitSec << "s)，未找到路径或未尝试。\n";
        else
    ...
    if (timeProvided)
    {
        ...
        std::cout << "运行时间(目标/实际): " << timeLimitSec << " / " << totalSec << " s\n";
        std::cout << "采样次数(实际): " << sampled << "\n";
    }
    else
    {
