        ...
        if (entry.is_regular_file()) {
            ...
            for (size_t batch_start = 0; batch_start < total_configs; batch_start += batch_size) {
                ...
                for (size_t config_idx = 0; config_idx < current_batch_size; ++config_idx) {
                    ...
                    jointAngles["panda_joint1"] = joint_angles[0];
                    jointAngles["panda_joint2"] = joint_angles[1];
                    jointAngles["panda_joint3"] = joint_angles[2];
                    jointAngles["panda_joint4"] = joint_angles[3];
                    jointAngles["panda_joint5"] = joint_angles[4];
                    jointAngles["panda_joint6"] = joint_angles[5];
