...
int main(){
    ...
    std::ofstream outfile(BACON_SOURCE_DIR "/data/top_fk_results.txt");
    if (!outfile.is_open()) {
        std::cerr << "Failed to open the file." << std::endl;
        return 1;
    }

    std::ofstream jointfile(BACON_SOURCE_DIR "/data/fk_top_joint_file.txt");
    if (!outfile.is_open()) {
        std::cerr << "Failed to open the file." << std::endl;
        return 1;
    }
    ...
    for(int i = 0; i < 1; i++){
        ...
        Eigen::Vector3d pvector = Eigen::Vector3d(0, 0, 0);
        Eigen::Matrix3d pmatrix = Eigen::Matrix3d::Identity();

        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                outfile << "0x" << std::setw(8) << to_fixed_point(pmatrix(i, j)) << " ";
            }
        }

        for (int i = 0; i < 3; ++i) {
            outfile << "0x" << std::setw(8) << to_fixed_point(pvector(i)) << " ";
        }
        ...
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                outfile << "0x" << std::setw(8) << to_fixed_point(model.links["prbt_flange"].rotation.toRotationMatrix()(i, j)) << " ";
            }
        }

        for (int i = 0; i < 3; ++i) {
            outfile << "0x" << std::setw(8) << to_fixed_point(model.links["prbt_flange"].position(i)) << " ";
        }
    }

    jointfile.close();
    outfile.close();
    std::cout << "Random tree traversal test data and results have been written to top_fk_results.txt" << std::endl;
}