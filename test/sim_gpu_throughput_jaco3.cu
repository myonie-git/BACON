...
int main(int argc, char *argv[]) {
    ...
    URDFModel model;
    std::string urdfFilePath = BACON_SOURCE_DIR "/urdf_gpu/j2n6s300_standalone.urdf";
        ...
        if (entry.is_regular_file()) {
            ...
            for (size_t batch_start = 0; batch_start < total_configs; batch_start += batch_size) {
                ...
                for (size_t config_idx = 0; config_idx < current_batch_size; ++config_idx) {
                    ...
                    jointAngles["j2n6s300_joint_1"] = joint_angles[0];
                    jointAngles["j2n6s300_joint_2"] = joint_angles[1];
                    jointAngles["j2n6s300_joint_3"] = joint_angles[2];
                    jointAngles["j2n6s300_joint_4"] = joint_angles[3];
                    jointAngles["j2n6s300_joint_5"] = joint_angles[4];
                    jointAngles["j2n6s300_joint_6"] = joint_angles[5];
