...
def count_lines(file_path):
    """
    统计文件中包含 'Env' 和 'No' 的行数。

    参数:
    - file_path (str): 文件路径。

    返回:
    - dict: 包含 'Env' 和 'No' 的行数。
    """
    counts = {'Env': 0, 'No': 0}
    try:
...
def generate_tables(base_dir='data'):
    """
    遍历指定目录下的所有 .txt 文件，统计每个文件中包含 'Env' 和 'No' 的行数，
    并生成两个表格分别展示这些统计数据。
    ...
    for robot in robot_models:
        ...
        for txt in txt_files:
            ...
            data_env[robot][env_size] = counts['Env']
            data_no[robot][env_size] = counts['No']
    ...
    print("\n包含 'No' 的行数统计：")
    print(tabulate(df_no, headers='keys', tablefmt='pretty'))

    # 将结果保存为 CSV 文件
    df_env.to_csv('sphere_env_counts.csv')
    df_no.to_csv('sphere_no_counts.csv')
    print("\n统计结果已保存为 'sphere_env_counts.csv' 和 'sphere_no_counts.csv'。")

if __name__ == "__main__":
    # 如果需要，可以通过命令行参数传递基础目录
    import argparse

    parser = argparse.ArgumentParser(description="统计每个 .txt 文件中包含 'Env' 和 'No' 的行数，并生成表格。")
    parser.add_argument(
        '-d', '--directory',
        type=str,
        default=f'{BACON_REPO_ROOT}/exp/collision_rate_sphere',
