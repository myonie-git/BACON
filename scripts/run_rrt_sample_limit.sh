...
# 统一 seed 下，批量跑 sim_rrt_sample_limit 的六个机械臂与多组 samples。
...
SEED="${SEED:-123456}"
OUT_CSV="${OUT_CSV:-${REPO}/scripts/rrt_sample_limit_results.csv}"
...
samples=(20 50 100 200 300 400 500 1000 2500 5000 8000 10000)
...
for robot in "${robots[@]}"; do
  for n in "${samples[@]}"; do
    ...
    mv "${log_file}" "${REPO}/scripts/log_sim_rrt_${robot}_${n}.log"
  done
done
